...
void removeLFU(PT *pageTable)
{
	...
	for (i = 0; i < pageTable->count; i++)
	{
		if (currentPTE->numRequests < LFU->numRequests)
			LFU = currentPTE;

		currentPTE = currentPTE->next;
	}

	LFU->next->prev = LFU->prev;
	LFU->prev->next = LFU->next;
	free(LFU);
...
void updateMRU(int page, PT *pageTable)
{
	...
	currentPTE->numRequests = currentPTE->numRequests + 1;
